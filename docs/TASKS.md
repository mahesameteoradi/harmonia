...
- [x] **T1.1** — `scanner/auth.py`: login sekali, simpan session ke `~/.harmonia/session.json` (chmod 600)
- [x] **T1.2** — `scanner/tags.py`: baca metadata via mutagen dengan fallback chain lengkap
- [x] **T1.3** — `file_hash` = md5(1MB pertama + filesize), lihat ADR-004
- [x] **T1.4** — **Preflight**: total ukuran, tandai file > 50 MB, bandingkan sisa kuota, tolak kalau melebihi
- [x] **T1.5** — `--dry-run` yang benar-benar tidak menyentuh jaringan
- [x] **T1.6** — Upsert `artists` → `albums` → `tracks` dengan status `pending`
- [x] **T1.7** — Upload audio ke Storage (4 worker, retry backoff) → update status `uploaded`
- [x] **T1.8** — Ekstrak cover, resize 500×500 q80, upload ke `covers` — satu per album
- [ ] **T1.9** — Opsi `--transcode opus` via ffmpeg lokal untuk file yang melebihi 50 MB
- [x] **T1.10** — `python -m scanner status`: pemakaian storage vs kuota 1 GB
- [x] **T1.11** — `python -m scanner cleanup`: hapus baris `pending` yang filenya tidak ada di Storage
...
- [x] **T2.1** — `public/js/supabase.js` + `storage.js` (signed URL, cache Map, batch `createSignedUrls`)
- [x] **T2.2** — `index.html`: shell single-page (sidebar, main, player bar bawah)
- [x] **T2.3** — Render library dari view `v_tracks`, paginated `.range()` 50 baris
- [x] **T2.4** — Kelas `Player`: play, pause, next, prev, seek, volume
- [x] **T2.5** — Queue: shuffle (Fisher-Yates), repeat off/all/one
- [x] **T2.6** — Progress bar draggable (abaikan `timeupdate` selama drag)
- [x] **T2.7** — **Pemulihan signed URL kedaluwarsa**: tangani `MEDIA_ERR_NETWORK`, buat URL baru, lanjut dari `currentTime`
- [x] **T2.8** — Prefetch URL lagu berikutnya (URL saja, bukan audionya)
- [ ] **T2.9** — Search dengan debounce 300ms → RPC `search_tracks`
- [x] **T2.10** — Media Session API (metadata + action handler)
- [x] **T2.11** — Playlist lokal: buat, tambah, hapus, reorder via RPC `reorder_playlist`
- [ ] **T2.12** — Halaman **Usage**: total storage terpakai, jumlah lagu, peringatan mendekati 1 GB
- [x] **T2.13** — CSS: layout responsif, tema gelap, cover besar di player bar
...
- [x] **T3.1** — Halaman Import: textarea untuk paste + drop zone untuk file CSV
- [x] **T3.2** — `parser.js`: deteksi format otomatis (CSV berheader / URI / teks biasa)
- [x] **T3.3** — Parser CSV yang benar — tangani field berkutip yang mengandung koma, BOM, dan `\r\n`
- [x] **T3.4** — Pencocokan kolom berbasis pola (`Track Name`, `Title`, `Song`, `Artist Name`, dst)
- [x] **T3.5** — Normalisasi durasi: `"3:45"`, detik, dan milidetik → ms
- [x] **T3.6** — **Preview 5 baris + tombol "Tukar kolom"** sebelum import dieksekusi
- [x] **T3.7** — Simpan hasil konfirmasi ke `imported_tracks`
...
- [x] **T3.15** — Panduan di UI: cara mendapatkan daftar dari Spotify desktop, Exportify, TuneMyMusic
